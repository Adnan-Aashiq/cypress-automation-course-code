...
describe('Automating lead forms', function () {
    sifmTestData.InspectionRequest.forEach((data) => {
        it.only('Placing PakWheels Car Inspection lead', function () {
            Homepageobj.openHomePage();
            Homepageobj.closeBanner();
            Homepageobj.ClickOnSignIn();
            Loginpageobj.loginWithEmail('sprint180@mailinator.com', '1234567')
            Homepageobj.clickOnCarInspection();
            Carinspectionleadformobj.location(data.city,data.cityArea,data.address)
            Carinspectionleadformobj.make_model_version(data.year,data.make,data.model,data.version)
            Carinspectionleadformobj.inspection_slot()
            //Carinspectionleadformobj.ClickOnSubmit()
            Carinspectionleadformobj.ordersummary()
