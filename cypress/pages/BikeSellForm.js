...
export class BikeSellForm extends SellForms{
    ...
    selectCity(city){
        ...
        cy.get('div[id="used_bike_ad_listing_attributes_city_id_chzn"] ul[class="chzn-results"]')
        .contains(city).click()
    }

    selectCityArea(city_area){
        cy.get('#used_bike_ad_listing_attributes_city_area_id_chzn > .chzn-single > span').click()
        cy.get('div[id="used_bike_ad_listing_attributes_city_area_id_chzn"] ul[class="chzn-results"]')
        .contains(city_area).click()
    }
